      ...
      >
        <div>
          <h3 className={`text-sm font-extrabold flex items-center gap-2 ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>
            ...
            <span>Risk Velocity Timeline ({timeRange === '7' ? '7-Day' : '14-Day'} Volume)</span>
          </h3>
        ...
        <div
          className={`inline-flex items-center p-0.5 rounded-xl border text-xs self-start sm:self-auto ${
            isLight ? 'bg-slate-100 border-slate-200' : 'bg-[#0f1117] border-[#232738]'
          }`}
        >
          <button
            onClick={() => setTimeRange('7')}
            className={`px-3 py-1.5 rounded-lg transition-all cursor-pointer font-bold min-h-[36px] ${
              timeRange === '7'
