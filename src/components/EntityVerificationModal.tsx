...
export const EntityVerificationModal: React.FC<EntityVerificationModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div
        className={`border rounded-2xl max-w-3xl w-full p-6 shadow-2xl space-y-6 my-8 animate-in fade-in zoom-in duration-200 ${
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className={`text-lg font-bold font-mono ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>
                  Device Telematics & Official Registry Verification
                </h3>
                <span className="px-2 py-0.5 text-xs font-mono rounded bg-red-500/10 text-red-500 border border-red-500/20 font-bold">
                  LIVE COMPLIANCE AUDIT
                </span>
              </div>
              <p className={`text-xs ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>
                Case ID: <span className="font-mono text-indigo-600 dark:text-indigo-400 font-bold">{patternId}</span> | Target Account:{' '}
                <span className={`font-mono font-semibold ${isLight ? 'text-slate-800' : 'text-slate-200'}`}>{entity.account_id}</span>
              </p>
            </div>
          </div>
